...
<div class="space-y-6 animate-in fade-in duration-500">
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
            ...
            <p class="text-gray-500">Kelola data Sales</p>
        </div>
    ...
    <form method="GET" action="{{ route('sales.index') }}" class="flex gap-2">
        ...
        @if(request('search') || request('cabang'))
            <a href="{{ route('sales.index') }}" class="px-4 py-2 bg-gray-200 rounded-xl hover:bg-gray-300">Reset</a>
        @endif
    </form>
            ...
            <table class="w-full text-left">
                <thead class="bg-gray-50 border-b border-gray-100">
                    <tr>
                        ...
                        <th class="px-6 py-4 font-semibold text-sm">NIP</th>
                        <th class="px-6 py-4 font-semibold text-sm">Nama Sales</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50 transition">
                        ...
                        <td class="px-6 py-4 text-sm">{{ $item->nama_sales }}</td>
                        @if(Auth::user()->cabang == "Pusat" && Auth::user()->role == "admin")
                        ...
                        <td class="px-6 py-4 flex justify-center gap-2">
                            ...
                            <button onclick="openDeleteModal('{{ route('sales.destroy', $item->id) }}')" class="text-red-600 hover:text-red-800">
...
<div id="modal-add" class="fixed inset-0 z-50 flex items-center justify-center bg-gray-900/50 hidden p-4">
    <div class="bg-white rounded-3xl w-full max-w-lg p-8 shadow-2xl">
        ...
        <form action="{{ route('sales.store') }}" method="POST" class="space-y-4">
            @csrf
            <div>
                <input type="text" name="nip" placeholder="Nomor Induk Pegawai" class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:ring-2 focus:ring-blue-500 outline-none">
                @error('barang') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
            </div>
            <input type="text" name="nama_sales" placeholder="Nama Sales" class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:ring-2 focus:ring-blue-500 outline-none">
...
<div id="modal-edit" class="fixed inset-0 z-50 flex items-center justify-center bg-gray-900/50 hidden p-4">
    <div class="bg-white rounded-3xl w-full max-w-lg p-8 shadow-2xl">
        ...
        <form id="editForm" method="POST" class="space-y-4">
            @csrf @method('PUT')
            <input type="text" name="nip" id="edit-nip" class="w-full px-4 py-3 rounded-xl border border-gray-200 outline-none">
            <input type="text" name="nama_sales" id="edit-nama" class="w-full px-4 py-3 rounded-xl border border-gray-200 outline-none"> 
            <div class="flex justify-end gap-3 mt-6">
...
<div id="modal-delete" class="fixed inset-0 z-50 flex items-center justify-center bg-gray-900/50 hidden p-4">
    <div class="bg-white rounded-3xl w-full max-w-sm p-8 text-center shadow-2xl">
        <h2 class="text-xl font-bold mb-4">Keluarkan Sales?</h2>
        <p class="text-gray-500 mb-6">Apakah Anda yakin ingin mengeluarkan sales ini?</p>
...
<script>
    ...
    function openEditModal(item) {
        document.getElementById('editForm').action = '/sales/' + item.id;
        document.getElementById('edit-nip').value = item.nip;
        document.getElementById('edit-nama').value = item.nama_sales;
